...
const HOLD_DURATION := 0.42
const EXIT_DURATION := 0.2
const PLAYER_TINT := Color(0.68, 0.9, 1.0, 1.0)
const OPPONENT_TINT := Color(1.0, 0.75, 0.75, 1.0)
const CARD_CENTER_POSITION := Vector2(180, 292)
...
func show_spell_preview(card_id: String, card_data: Dictionary, caster: String) -> void:
	...
	caster_label.modulate = PLAYER_TINT if caster == "player" else OPPONENT_TINT

	active_card_preview = CARD_PREVIEW_SCENE.instantiate()
	preview_anchor.add_child(active_card_preview)
	active_card_preview.position = CARD_CENTER_POSITION
	...
	active_tween.tween_property(backdrop, "color", Color(0.03, 0.05, 0.09, 0.34), ENTRANCE_DURATION)
	active_tween.tween_property(preview_anchor, "modulate:a", 1.0, ENTRANCE_DURATION)
	active_tween.tween_property(preview_anchor, "scale", Vector2.ONE, ENTRANCE_DURATION).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	active_tween.tween_property(preview_anchor, "position", base_preview_position, ENTRANCE_DURATION).set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
	...
	active_tween.set_pause_mode(Tween.TWEEN_PAUSE_PROCESS)
	active_tween.set_parallel(true)
	active_tween.tween_property(backdrop, "color", Color(0.03, 0.05, 0.09, 0.0), EXIT_DURATION)
	active_tween.tween_property(preview_anchor, "modulate:a", 0.0, EXIT_DURATION)
	active_tween.tween_property(preview_anchor, "scale", Vector2(1.08, 1.08), EXIT_DURATION).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	active_tween.tween_property(preview_anchor, "position", base_preview_position + Vector2(0, -18), EXIT_DURATION).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	await active_tween.finished
